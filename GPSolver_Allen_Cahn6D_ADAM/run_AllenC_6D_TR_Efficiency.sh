...
store_path=$SCRIPT_DIR"/result/Data_AllenC6D_TR_Efficiency"
model_path=$SCRIPT_DIR"/training_process_store_pth/model_AllenC6D_TR_Efficiency"
...
do
    ...
    do
        python GP_S.py --lr=0.01 --lr-gap=1000 --lr-decay=0.8 --epochs=1000000 --early-stop=20000 --log-store-path=$store"/config"$count \
         ...
         --x-range="0.0,1.0" --n-xtrain ${trainGrid[$nxdx]} ${trainGrid[$nxdx]} ${trainGrid[$nxdx]} ${trainGrid[$nxdx]} ${trainGrid[$nxdx]} ${trainGrid[$nxdx]} --test-batch=1000000 --method=3 \
