...
def boundary_gen(args, x_range):
    sampler = LatinHypercube(args.n_order-1, optimization='random-cd')
...
def test_gen(args, x_range):
    sampler = LatinHypercube(args.n_order, optimization='random-cd')
    X_test = x_range[0] + sampler.random(args.n_test) * (x_range[-1] - x_range[0])
    if args.n_order > 2:
        Y_test = u_ground_truth(args, np.concatenate((X_test, X_test[:, 0:1]), axis=-1))
    else:
        Y_test = u_ground_truth(args, X_test)
...
if __name__ == '__main__':
    ...
    X_test, Y_test = test_gen(args, x_range)
