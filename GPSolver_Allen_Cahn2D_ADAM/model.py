...
class GP_TD(object):
    ...
    def loss(self, params, key):
        ...
        log_ll_eq = jnp.sum(jnp.square(u_ddx1.reshape(-1, 1) + u_ddx2.reshape(-1, 1) + gamma * (u.reshape(-1, 1)**m - u.reshape(-1, 1)) - force.reshape(-1, 1)))
        
        # boundary conditions
        u_b = vmap(self.to_tensor, (None, 0, 0))(params, self.data_gen.x_b[:, 0], self.data_gen.x_b[:, 1])
        log_ll_boundaries = jnp.sum(jnp.square(u_b.reshape(-1) - jnp.array(self.data_gen.y_b.reshape(-1))))
